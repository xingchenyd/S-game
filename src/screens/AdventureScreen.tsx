...
export default function AdventureScreen({ profile, onChange, onImmersive }: Props) {
  ...
  const [metrics, setMetrics] = useState<RunMetrics>({ hp: 100, maxHp: 100, pollution: 68, value: 0, accuracy: 100, combo: 0, stage: 0, totalStages: 5 })
  ...
  const begin = () => { setPhase('combat'); setMetrics((m) => ({ ...m, stage: 1 })); onImmersive(true) }
  ...
  const chooseBuild = (id: string) => { setBuilds((value) => [...value, id]); setPhase('boss'); setMetrics((m) => ({ ...m, stage: 4 })) }
  const finish = (result: BattleResult) => {
    ...
    const prototypeId = selected.id === 'lujiazui-circuit' ? 'lujiazui-core' : selected.id === 'suzhou-plastic' ? 'suzhou-core' : selected.id === 'yangpu-paper' ? 'yangpu-core' : 'changning-core'
    const next = {
      ...profile,
  ...
  const reset = () => { setSelected(null); setPhase('briefing'); setBuilds([]); setAnswer(null); setCorrect(false); setPurified(0); setMetrics({ hp: 100, maxHp: 100, pollution: 68, value: 0, accuracy: 100, combo: 0, stage: 0, totalStages: 5 }); onImmersive(false) }
  ...
  if (!selected) return <div className="adventure-select screen-enter">
    ...
    <div className="adventure-grid">{adventures.map((item, index) => <button key={item.id} className={`adventure-card ${!item.available ? 'locked' : ''}`} style={{ '--accent': item.accent } as React.CSSProperties} onClick={() => item.available && setSelected(item)} disabled={!item.available}>
      <img src={item.background} alt="" /><span className="adventure-shade" /><span className="route-number">0{index + 1}</span>{!item.available && <span className="locked-chip"><LockKeyhole /> 筹备中</span>}
      <span className="adventure-info"><span className="eyebrow">{item.location}</span><b>{item.name}</b><small>{item.briefing}</small><span className="boss-line"><ShieldAlert /> BOSS · {item.boss}</span></span><ChevronRight className="route-arrow" />
    </button>)}</div>
  </div>
  ...
  return <div className="run-screen screen-enter">
    ...
    {(phase === 'combat' || phase === 'boss') && <>
      <div className="objective-chip"><Target /> {phase === 'boss' ? `击破 ${selected.boss} 的污染外壳` : '稳定区域，净化游离污染体'} <b>{metrics.combo}</b></div>
      <PhaserCombat adventure={selected} difficulty={difficulty} boss={phase === 'boss'} builds={builds} initialPollution={metrics.pollution} initialValue={metrics.value} onHud={(value) => setMetrics((m) => ({ ...m, ...value }))} onComplete={battleDone} onDefeat={() => { setPhase('defeat'); onImmersive(false) }} />
    </>}

    {phase === 'classify' && <section className="decision-overlay">
      <div className="decision-card pixel-panel"><span className="eyebrow">处理节点 · 不是考试，是行动选择</span><div className="item-orb">{challenge.icon}</div><h2>{challenge.item}</h2><p>{challenge.prompt}</p>
        <div className="decision-options">{challenge.options.map((option, index) => <button key={option} className={answer === null ? '' : index === challenge.answer ? 'correct' : answer === index ? 'wrong' : 'muted'} onClick={() => submitAnswer(index)}>{String.fromCharCode(65 + index)}. {option}</button>)}</div>
        {answer !== null && <div className={`answer-feedback ${correct ? 'correct' : 'wrong'}`}><b>{correct ? '处理正确：价值被保留下来' : '本次处理仍有风险'}</b><p>{challenge.explain}</p><button className="primary-button" onClick={() => setPhase('build')}>进入构筑选择 <ChevronRight /></button></div>}
      </div>
    </section>}
