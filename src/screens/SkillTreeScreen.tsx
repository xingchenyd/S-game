...
export default function SkillTreeScreen({ profile, onChange, notify }: Props) {
  const unlock = (id: string) => {
    ...
    if (profile.skillPoints < skill.cost) return notify('技能点不足：首次完成剧情得1点，训练首次达到80分得2点，主线关键节点也会奖励。', 'warning')
  ...
  return <div className="skill-screen screen-enter">
    ...
    <div className="skill-branches">{branches.map((branch) => { const branchSkills = skillNodes.filter((skill) => skill.branch === branch); const mastered = branchSkills.filter((skill) => profile.unlockedSkills.includes(skill.id)).length; return <section key={branch} className={`skill-branch branch-${branch}`}>
      <header><GameIcon name={branch === '行动' ? 'training' : branch === '系统' ? 'knowledge' : 'theater'} size={58} /><div><small>{branch === '行动' ? 'ACTION' : branch === '系统' ? 'SYSTEM' : 'EMPATHY'}</small><h2>{branch}分支</h2><em>{mastered} / {branchSkills.length} 已掌握</em></div></header>
      <div className="skill-track">{branchSkills.map((skill, index) => {
    ...
    <aside className="skill-note"><b>设计约束</b><span>技能点不能购买，只能通过主线与重要教育节点获得。每条分支8层、总成本27点。首次完成每篇剧情获得1技能点，任一训练首次达到80分获得2点，上海主线关键节点仍会奖励；总量足以成长，但三条分支的先后顺序由你决定。</span></aside>
  </div>
}
